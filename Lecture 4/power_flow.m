...
function [V,P_gen,Q_gen,S_branch] = power_flow(System)
...
% Proračun aktivnih i reaktivnih snaga generatora

Generator_Buses = [find(Bus_Type==1) ; find(Bus_Type==2)] ; 
P_gen = P + P_load ; 
Q_gen = Q + Q_load ;
P_gen = P_gen(Generator_Buses) ; 
Q_gen = Q_gen(Generator_Buses) ; 

% Proračun aktivnih i reaktivnih snaga po granama mreže

V = V.*exp(sqrt(-1)*Theta) ; 
From_Bus = System.Branches.From_Bus ; 
To_Bus = System.Branches.To_Bus ;
Z_branch = System.Branches.R+sqrt(-1)*System.Branches.X ;
Y_branch = 1./Z_branch ; 
S_branch = V(From_Bus).*conj(Y_branch.*(V(From_Bus)-V(To_Bus))) ; 
